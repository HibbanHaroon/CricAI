...
@app.get('/')
async def root(url: str, sessionId: str, videoName: str, shotType: str):
    # Pose Estimation - get angles of the player's landmarks from the video of a session
    angles = get_angles(url)
    
    # For now, the analysis video is the video returned after performing pose estimation on the player's video.
...
async def get_ideal_angles(url: str):
    ...
    return {'ideal_angles': ideal_angles}
